...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Password Hashing (if new password is provided)
        ...
        if ($stmt->execute()) {
            ...
            $error = "Failed to update profile.";//SQL query အောင်မြင်စွာ run မဖြစ်ခဲ့ရင် $error variable ထဲမှာ Failed to update profile. ဆိုတဲ့ error message ကို assign လုပ်တယ်။
...
<head>
    ...
    <style>
        ...
        .form-control 
        {
            border-radius: 8px !important; /* Add !important to force override */
            box-shadow: 0 4px 6px rgba(0, 0, 0, 0.2) !important; /* Add shadow */
            border: 1px solid #000 !important; /* Add custom border */
        ...
        .profile-image-wrapper img 
        {
            ...
            box-shadow: 0 1px 7px rgba(48, 46, 46, 0.3); /* Subtle shadow around the image */
            display: block; /* Removes any unwanted inline spacing */
            margin: 0 auto; /* Centers the image if required */
       ...
       .profile-image-wrapper img:hover 
      {
        ...
        {
            border-radius: 8px 0 0 0; /* Rounded top-left and bottom-left corners only */
            border: 1px solid #000; /* Match border styling */
      ...
      .input-group .toggle-password 
       {
        border-radius: 0 8px 8px 0; /* Rounded top-right and bottom-right corners only */
        border: 1px solid #000; /* Match border styling */
                ...
                <form  method="POST" enctype="multipart/form-data">
                    <div class="row mb-4">
                        <div class="col-md-7">
                            ...
                            <input type="text" class="form-control" id="firstName" name="first_name" value="<?php echo htmlspecialchars($user['first_name']); ?>" readonly>
                        </div>
                        <div class="col-md-7">
                            <label for="lastName" class="form-label">Last Name</label>
                            <input type="text" class="form-control" id="lastName" name="last_name" value="<?php echo htmlspecialchars($user['last_name']); ?>" readonly>
                        </div>
                    </div>
                    <div class="row mb-4">
                        <div class="col-md-7">
                            <label for="emailAddress" class="form-label">Email Address</label>
                            <input type="email" class="form-control" id="emailAddress" name="email" value="<?php echo htmlspecialchars($user['email']); ?>" readonly>
                        </div>
                        <div class="col-md-7">
